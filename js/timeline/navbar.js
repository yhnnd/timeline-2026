...
function getBrowserLogo(browserName) {
    ...
    const logoFuncs = {
        "firefox": getFirefoxLogo,
        "chrome": getChromeLogo,
        "safari": getSafariLogo,
        "edge": getEdgeLogo,
    };
    const logoRenderer = logoFuncs[browserName];
    if (typeof logoRenderer === "function") {
        return logoRenderer();
    }
}


function getFirefoxLogo() {
    return `<svg width="77.42" height="79.97" version="1.1" viewBox="0 0 77.42 79.97" xmlns="http://www.w3.org/2000/svg" xmlns:cc="http://creativecommons.org/ns#" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#">
   <defs>
...
function getEdgeLogo() {
    return `<svg width="77" height="77" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256">
    <defs>
        <radialGradient id="b" cx="161.8" cy="68.9" r="95.4" gradientTransform="matrix(1 0 0 -.95 0 248.8)" gradientUnits="userSpaceOnUse"><stop offset=".7" stop-opacity="0"/><stop offset=".9" stop-opacity=".5"/><stop offset="1"/></radialGradient>
        <radialGradient id="d" cx="-340.3" cy="63" r="143.2" gradientTransform="matrix(.15 -.99 -.8 -.12 176.6 -125.4)" gradientUnits="userSpaceOnUse"><stop offset=".8" stop-opacity="0"/><stop offset=".9" stop-opacity=".5"/><stop offset="1"/></radialGradient>
        <radialGradient id="e" cx="113.4" cy="570.2" r="202.4" gradientTransform="matrix(-.04 1 2.13 .08 -1179.5 -106.7)" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#35c1f1"/><stop offset=".1" stop-color="#34c1ed"/><stop offset=".2" stop-color="#2fc2df"/><stop offset=".3" stop-color="#2bc3d2"/><stop offset=".7" stop-color="#36c752"/></radialGradient>
        <radialGradient id="f" cx="376.5" cy="568" r="97.3" gradientTransform="matrix(.28 .96 .78 -.23 -303.8 -148.5)" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#66eb6e"/><stop offset="1" stop-color="#66eb6e" stop-opacity="0"/></radialGradient>
        <linearGradient id="a" x1="63.3" y1="84" x2="241.7" y2="84" gradientTransform="matrix(1 0 0 -1 0 266)" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#0c59a4"/><stop offset="1" stop-color="#114a8b"/></linearGradient>
        <linearGradient id="c" x1="157.3" y1="161.4" x2="46" y2="40.1" gradientTransform="matrix(1 0 0 -1 0 266)" gradientUnits="userSpaceOnUse"><stop offset="0" stop-color="#1b9de2"/><stop offset=".2" stop-color="#1595df"/><stop offset=".7" stop-color="#0680d7"/><stop offset="1" stop-color="#0078d4"/></linearGradient>
    </defs>
...
function deepCopy(obj, maxLevel = 3, level = 0) {
    // if obj is a circular structure, copy deep level is limited.
    // if obj is a regular object, copy deep level is unlimited.
    if (level >= maxLevel) {
        return "[Too Deep]";
    }
    ...
    try {
        ...
        for (const [key, value] of Object.entries(obj)) {
            if (typeof value === "object") {
                dst[key] = deepCopy(value, maxLevel, level + 1);
            } else {
                dst[key] = value;
            }
        }
    ...
    if (objStr === "{}") {
        // JSON.stringify(window.navigator) is "{}"
        return obj;
    } else {
...
function renderDomText() {
    const tags = document.querySelectorAll("[data-inner-text]");
    for (const tag of tags) {
        (function (window) {// sandbox
            tag.innerText = eval(tag.getAttribute("data-inner-text"));
        })(deepCopy(window));
    }
}

function renderDomHtml() {
    const tags2 = document.querySelectorAll("[data-inner-html]");
    for (const tag of tags2) {
        (function (window) {// sandbox
            tag.innerHTML = eval(tag.getAttribute("data-inner-html"));
        })(deepCopy(window));
    }
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (browserNames.includes(webBrowserName)) {
        const wrapper = document.getElementById(("web-browser-logo"));
        if (wrapper) {
            wrapper.innerHTML = window.getBrowserLogo(webBrowserName);
        }
    }
    renderDomText();
    renderDomHtml();
